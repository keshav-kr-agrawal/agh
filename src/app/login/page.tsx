...
import { Phone, User, ArrowRight, Mail, Lock, ShieldCheck, Eye, EyeOff } from 'lucide-react';
...
export default function CustomerLoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (authMode === 'email') {
      ...
      const extractedName = name || email.split('@')[0];
      loginCustomer(phone || '+91 98765 43210', extractedName, email);
    } else {
      ...
      <div className="w-full max-w-md bg-cream border border-cream-border rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-espresso/60">
            Log in to manage orders, live parcel tracking, and shopping cart
          </p>
        </div>

        {/* Auth Mode Toggle */}
        <div className="flex bg-cream-muted p-1 rounded-xl border border-cream-border text-xs font-bold">
          <button
            type="button"
            onClick={() => setAuthMode('email')}
            className={`flex-1 py-2 rounded-lg transition ${authMode === 'email' ? 'bg-terracotta text-cream shadow-sm' : 'text-espresso/70'}`}
          >
            Email & Password
          </button>
          <button
            ...
            className={`flex-1 py-2 rounded-lg transition ${authMode === 'phone' ? 'bg-terracotta text-cream shadow-sm' : 'text-espresso/70'}`}
          >
            Mobile Number
          </button>
        </div>
        ...
        <div className="relative flex py-1 items-center">
          ...
          <span className="flex-shrink mx-3 text-[10px] text-espresso/40 uppercase font-mono">Or Continue With</span>
          <div className="flex-grow border-t border-cream-border"></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          {authMode === 'email' ? (
            ...
            <>
              <div>
                <label className="block font-bold text-espresso mb-1 flex items-center gap-1.5">
                  <User className="w-3.5 h-3.5 text-terracotta" /> Your Name *
                </label>
                <input
                  type="text"
                  required
        ...
        <div className="pt-4 border-t border-cream-border text-center text-xs text-espresso/60 space-y-2">
          <p>Don't have an account? <Link href="/signup" className="text-terracotta font-bold hover:underline">Sign Up</Link></p>
